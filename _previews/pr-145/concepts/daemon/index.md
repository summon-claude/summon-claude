...
1. **Drain sessions**: `session_manager.shutdown()` signals all sessions to stop and waits up to 30 seconds (`_SHUTDOWN_WAIT_TIMEOUT`). Sessions that do not exit within this window are cancelled.
1. **Stop Bolt**: `bolt_router.stop()` closes the Slack Socket Mode connection.
