...
1. Fetches existing tags
1. Shows the current version and prompts for the new version (X.Y.Z format)
1. Validates the version is a valid semver and not a downgrade
...
1. The `release: published` event triggers the `publish.yaml` workflow
1. The workflow checks out with `fetch-depth: 0` (required for hatch-vcs to read all tags)
1. `uv build` runs — `hatch-vcs` reads the tag → produces a versioned sdist and wheel
1. Artifacts are published to PyPI via OIDC trusted publisher (no stored API tokens)
1. After PyPI publish succeeds, a `repository_dispatch` event triggers the Homebrew tap update
...
1. Create a token with scopes:
1. **`repo`** — create branches, update files, create/update PRs
1. **`workflow`** — update pinned action SHAs in `.github/workflows/`
1. Add as repo secret: **GitHub repo → Settings → Secrets and variables → Actions → New repository secret**
1. Name: `RENOVATE_TOKEN`
1. Value: the PAT
...
1. Computes SHA-256 checksums
1. Updates the formula with the new version and checksums
1. Opens a PR (or auto-merges, depending on tap config)

**Homebrew and sdist vs. wheel:** `brew update-python-resources` prefers sdists over wheels by default. For packages with broken sdists (e.g., missing files), the tap's `prefer-wheels.py` script swaps specific packages to use their wheels instead. The list of packages with broken sdists is maintained in the tap's update workflow.
