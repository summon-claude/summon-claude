...
1. The message posts as ephemeral (visible only to the authenticated user) with **Approve** and **Deny** buttons.
1. A ping posts to the main channel to ensure the user gets a notification.
1. If no response arrives within 5 minutes (`_PERMISSION_TIMEOUT_S = 300`), the request is automatically denied.
