...
- **Jira integration** — Atlassian Rovo MCP integration with OAuth 2.1 (PKCE + dynamic client registration). Authenticate via `summon auth jira login` — opens a browser for OAuth consent, auto-discovers cloud sites, and stores credentials locally with 0600 permissions (#94)
- **Jira read-only enforcement** — Permission handler classifies all 31 Jira MCP tools: read-only prefixes (`get`, `search`, `lookup`) auto-approved, write tools and `fetchAtlassian` hard-denied, fail-closed on unknown tools. Defense-in-depth: hard-deny list checked before auto-approve prefixes (#94)
- **Jira PM triage** — PM agents scan Jira issues on each periodic cycle using per-project JQL filters (`summon project add --jql`, `summon project update --jql`). Canvas state tracking avoids re-alerting on previously triaged issues. Prompt injection defense for untrusted issue content (#94)
- **Jira scribe integration** — Jira MCP wired to scribe sessions with system prompt (untrusted-content warnings, Gmail/Jira dedup) and scan prompt (Jira monitoring for mentions, assignments, and status changes via JQL) (#94)
- **Jira DB migration** — Schema v15->v16 adds `jira_jql TEXT` column to `projects` table for per-project JQL issue filters (#94)
...
## [0.2.1](https://github.com/summon-claude/summon-claude/compare/v0.2.0...v0.2.1) - 2026-03-12
...
## [0.2.0](https://github.com/summon-claude/summon-claude/compare/v0.1.1...v0.2.0) - 2026-03-09
...
## [0.1.1](https://github.com/summon-claude/summon-claude/compare/v0.1.0...v0.1.1) - 2026-02-27
...
## [0.1.0](https://github.com/summon-claude/summon-claude/commits/v0.1.0) - 2026-02-25
