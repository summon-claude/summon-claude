...
1. PM calls `session_message` to tell `worker-1` to create a task for the review
1. `worker-1` calls `TaskCreate(content="Auth review", priority="high")`
1. PM periodically calls `TaskList(session_ids=["sess-worker-1"])` to check progress
1. PM updates the canvas Active Work table via `summon_canvas_update_section`
...
- [PM Agents](https://summon-claude.github.io/summon-claude/latest/guide/pm-agents/index.md) — how the PM coordinates tasks across sessions
- [Canvas](https://summon-claude.github.io/summon-claude/latest/guide/canvas/index.md) — how cron and task data appears on the canvas
- [Configuration](https://summon-claude.github.io/summon-claude/latest/guide/configuration/index.md) — session-level configuration options
