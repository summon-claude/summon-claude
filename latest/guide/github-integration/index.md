...
If your project has [workflow instructions](https://summon-claude.github.io/summon-claude/latest/guide/projects/#managing-workflow-instructions) that define pre-review steps (e.g., "run the test suite before review"), the PM follows those steps first and only spawns a reviewer after they pass.
...
1. Checks for bugs, security issues, logic errors, and style problems
1. Fixes issues directly — commits with descriptive messages and pushes to the PR's head branch
1. Runs the project's test suite before every push
1. Applies the "Ready for Review" label when satisfied
1. Posts a detailed summary of findings and fixes to its Slack channel
...
1. The reviewer enters a worktree using `EnterWorktree(name="review-pr{number}")`
1. Inside the worktree, the reviewer fetches and checks out the PR's head branch
1. All file reads and writes are constrained to the worktree directory
...
- [PM Agents](https://summon-claude.github.io/summon-claude/latest/guide/pm-agents/index.md) — spawning reviewer sessions
- [Sessions](https://summon-claude.github.io/summon-claude/latest/guide/sessions/index.md) — session lifecycle
- [Configuration](https://summon-claude.github.io/summon-claude/latest/guide/configuration/index.md) — full configuration reference
