...
google or slack-browser extras not found
...
Wrong Slack scopes — bot can't post or read messages
...
**Fix:** Go to **api.slack.com/apps → Your App → OAuth & Permissions → Scopes** and add the required bot token scopes. After adding scopes, reinstall the app to your workspace. See [Slack Setup](https://summon-claude.github.io/summon-claude/latest/getting-started/slack-setup/index.md) for the full scope list.

Messages sent but Claude never responds
...
1. Toggle **Enable Events** to **On** (Socket Mode means no Request URL is needed)
1. Expand **Subscribe to bot events** and verify these events are listed: `message.channels`, `message.groups`, `reaction_added`, `app_home_opened`, `file_shared`
1. Click **Save Changes**
1. If prompted, **reinstall the app** to the workspace

The manifest should set this automatically
...
Missing app-level token or connections:write scope
...
1. Create or select a token — it must have the `connections:write` scope.
...
Auth code locked after failed attempts
...
/summon not recognized in Slack
...
1. Clean up stale sessions: `summon session cleanup`
1. Check logs for errors: `summon session logs <session-name>`
1. Verify Claude CLI is available: `claude --version`
1. Validate config: `summon config check`

Stale daemon / can't start new session
...
Stale sessions in session list
...
Session list shows wrong status
...
Approval buttons not appearing in Slack
...
1. Enable interactivity: **api.slack.com/apps → Your App → Interactivity & Shortcuts → On**.
1. Ensure the bot is in the channel (`/invite @your-bot-name`).
...
**Cause:** Permission messages are normal messages that are intentionally deleted after interaction to keep the channel clean. A persistent confirmation (e.g. " Approved: `Edit`") is posted to the turn thread as an audit trail.

**Behavior:** This is by design. See [Permissions](https://summon-claude.github.io/summon-claude/latest/reference/permissions/index.md) for details.

______________________________________________________________________

## Canvas

Canvas not created on free Slack plan
...
1. If a canvas exists, summon will sync to it automatically once discovered.

Note: On the free plan, you cannot have more than one canvas per channel. Plan your channel usage accordingly.

Canvas not syncing / content outdated
...
Canvas edits trigger unwanted Slack channel notifications
...
Google OAuth flow fails or never completes
...
Google Workspace collector not working
...
Enterprise Grid Slack monitoring
...
`summon auth jira status` says 'no cloud_id'
...
Browser doesn't open during Jira login
...
'DCR failed' error during Jira login
...
Token refresh failures in session logs
...
Jira tools not available in session
...
1. Check config: `summon config check`
1. If status shows issues, re-authenticate: `summon auth jira login`
1. If the MCP server is unreachable, individual tool calls will return errors — Claude adapts automatically.
...
Project up doesn't resume sessions
