...
1. **Second line (optional):** Essential technical context if non-obvious
1. That's it.
...
1. Write an async migration function that takes `db: aiosqlite.Connection`
1. Add it to `_MIGRATIONS` keyed by the version it migrates *from*
...
1. Register it and add the subsystem name:
...
1. Add guard test mappings in `tests/test_diagnostics_guard.py`:
