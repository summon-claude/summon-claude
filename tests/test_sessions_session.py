...
class TestServerInfoModelCacheWiring:
    ...
    async def test_server_info_triggers_cache_refresh(self, registry):
        ...
        with (
            ...
            patch("summon_claude.sessions.session.reconcile_context_window_sizes"),
            contextlib.suppress(RuntimeError),
        ):
            await session._run_session_tasks(rt, AsyncMock())
    ...
    async def test_server_info_triggers_context_reconciliation(self, registry):
        """When server_info.models is non-empty, reconcile_context_window_sizes is called."""
        models = [{"value": "claude-opus-4-6"}]
        ...
        with (
            ...
            patch.object(session, "_run_preprocessor", fake_preprocessor),
            patch.object(session, "_run_response_consumer", fake_consumer),
            patch("summon_claude.cli.model_cache.cache_sdk_models"),
            patch(
