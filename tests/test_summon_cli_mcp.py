...
class TestMCPServerCreation:
    ...
    def test_tool_count_without_canvas(self, populated_registry):
        tools = create_summon_cli_mcp_tools(populated_registry, "sid", "uid", "cid", "/tmp")
        assert len(tools) == 7

    def test_tool_count_with_canvas(self, populated_registry):
        mock_canvas = AsyncMock()
        tools = create_summon_cli_mcp_tools(
            populated_registry, "sid", "uid", "cid", "/tmp", canvas_store=mock_canvas
        )
        assert len(tools) == 7
...
@pytest.fixture
def canvas_tools_no_store(populated_registry):
    """Tools fixture without a CanvasStore (canvas_store=None)."""
    return {
        t.name: t
        for t in create_summon_cli_mcp_tools(
            registry=populated_registry,
...
class TestCanvasUpdateSection:
    ...
    async def test_canvas_none_returns_error(self, canvas_tools_no_store):
        result = await canvas_tools_no_store["summon_canvas_update_section"].handler(
            {"heading": "Notes", "markdown": "something"}
        )
        assert result["is_error"] is True
        assert "not available" in result["content"][0]["text"].lower()
