...
class TestResetData:
    ...
    def test_reset_data_refuses_project_sessions_only(self):
        ...
        runner = CliRunner()
        mock_sessions = [{"session_id": "abc", "session_name": "pm-agent", "pm_profile": True}]
        with (
            patch("summon_claude.cli.reset.is_interactive", return_value=True),
            patch("summon_claude.cli.reset.is_daemon_running", return_value=True),
            patch(
    ...
    def test_reset_data_refuses_mixed_sessions(self):
        ...
        mock_sessions = [
            ...
            {"session_id": "def", "session_name": "pm-agent", "pm_profile": True},
        ]
        with (
            patch("summon_claude.cli.reset.is_interactive", return_value=True),
            patch("summon_claude.cli.reset.is_daemon_running", return_value=True),
    ...
    def test_reset_data_local_mode_cleans_up_socket(self, tmp_path):
        """'reset data' in local mode unlinks the socket and removes empty parent dirs."""
        data_dir = tmp_path / "data"
        data_dir.mkdir()
        (data_dir / "registry.db").touch()
        ...
        with (
            ...
            patch("summon_claude.cli.reset.get_data_dir", return_value=data_dir),
            patch("summon_claude.cli.reset.Path.home", return_value=tmp_path),
            patch("summon_claude.cli.reset.is_local_install", return_value=True),
            patch("summon_claude.cli.reset.get_socket_path", return_value=sock_file),
        ):
            result = runner.invoke(cli, ["reset", "data"], input="y\n")
    ...
    def test_reset_data_global_mode_skips_socket_cleanup(self, tmp_path):
        """'reset data' in global mode does not touch any socket file."""
        data_dir = tmp_path / "data"
        data_dir.mkdir()
        (data_dir / "registry.db").touch()

        sock_parent = tmp_path / "sockets"
        sock_parent.mkdir()
        sock_file = sock_parent / "daemon.sock"
        sock_file.touch()
        ...
        with (
            ...
            patch("summon_claude.cli.reset.get_data_dir", return_value=data_dir),
            patch("summon_claude.cli.reset.Path.home", return_value=tmp_path),
            patch("summon_claude.cli.reset.is_local_install", return_value=False),
        ):
            result = runner.invoke(cli, ["reset", "data"], input="y\n")

        assert result.exit_code == 0
        assert not data_dir.exists()
        assert sock_file.exists(), "global mode must not touch socket files"
