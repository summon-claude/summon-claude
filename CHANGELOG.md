...
- **Scribe agent configuration** — `SUMMON_SCRIBE_ENABLED`, `SUMMON_SCRIBE_MODEL`, `SUMMON_SCRIBE_GOOGLE_ENABLED`, `SUMMON_SCRIBE_GOOGLE_SERVICES`, and related config vars for the scribe monitoring agent. `summon config google-auth` / `summon config google-status` for Google Workspace OAuth (#41)
...
- **External Slack CLI** — `summon config slack-auth`, `summon config slack-status`, `summon config slack-remove` for browser-based Slack workspace authentication with 0o600 auth state files
