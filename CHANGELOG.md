...
- **Registry schema migrations** — Schema changes extracted into `sessions/migrations.py` as the single source of truth. Fresh databases create the v1 baseline and run all migrations. Migrations v1→v2 (spawn tokens, parent session), v2→v3 (workflow defaults table), v3→v4 (active name index), v4→v5 (canvas columns), v5→v6 (parent session index) (#39, #42, #45)
