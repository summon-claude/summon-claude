...
- **Jira scribe integration** — Jira MCP wired to scribe sessions with domain-specific system prompt (untrusted-content warnings) and Gmail dedup (skips Jira notification emails when both sources are active). Scribe scan prompt does not yet include Jira-specific scan instructions (#94)
